...
//        Publisher<GenerateContentResponse> streamingResponse = model.generateContentStream(content); // 现在应该可以识别 model 了
    ...
    private String buildAiPrompt(String userPreferences, List<Incense> incenseList) {
        ...
        return "### あなたのタスク：\n" +
                "ユーザーの選択条件に基づき、提供されたお香リストから、**厳密に**最も合致する3～5種類のお香を選び出してください。\n" +
                "完全に合致する5種類が見つからない場合は、最も近い3～5種類を選んでください。\n" +
                "\n" +
                "### ユーザーの要望：\n" + userPreferences + "\n" +
                "\n" +
                "### 利用可能な香リスト\n" + incenseJson + "\n" +
                "\n" +
                "### レスポンス形式:\n" +
                "**必ず **純粋なJSON配列**（追加の説明やテキストは不要）で返してください，形式は以下の通りです：\n" +
                "[\n" +
                "  {\"name\": \"お香A\", \"effect\": \"效果A\", \"material\": \"材料A\", \"imageUrl\": \"URL_A\", \"description\": \"説明A\", \"url\": \"購入リンクA\"},\n" +
                "  {\"name\": \"お香B\", \"effect\": \"效果B\", \"material\": \"材料B\", \"imageUrl\": \"URL_B\", \"description\": \"説明B\", \"url\": \"購入リンクB\"}\n" +
                "]\n" +
                "\n" +
                "⚠️ **注意**：\n" +
                "1. **説明や追加のテキストは一切含めず**、JSON配列のみを返してください\n" +
                "2. **ユーザーの要望に厳密に従って選択**し、**結果がユーザーの選択条件によって変わる**ようにしてください。**。\n" +
                "3. **毎回同じ商品を返すのではなく**、条件に最も合致する商品を選んでください。\n";
    }
